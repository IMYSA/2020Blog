...
public interface ArticleMapper {
    @Insert("INSERT INTO `article` (title, author, description, content_md, content_html, is_banned) VALUES (#{title}, #{author},#{description}, #{content_md},#{content_html},#{is_banned})")
    void addArticle(Article a);

    @Select("Select * from `article` where author = #{author} AND is_banned = 0")
    List<Article> getArticlesByAuthorId(int author);

    @Select("Select * from `article` where id = #{id} AND is_banned = 0")
